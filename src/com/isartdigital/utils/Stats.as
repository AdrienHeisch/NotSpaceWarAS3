 ...
 * 
 * Simplest:
 * this.GameContainer.instance.addChild(new Stats());
 *
 * Make it bigger, smallest possible value is 70.(width):
 * GameContainer.instance.addChild(new Stats(150));
 * 
 * Change initial position (x, y): 
 * GameContainer.instance.addChild(new Stats(150, 10, 20));
 *
 * Make it minimized(isMinimized):
 * GameContainer.instance.addChild(new Stats(150, 10, 20, true));
 * 
 * Make it not draggable(isDraggable):
 * GameContainer.instance.addChild(new Stats(150, 10, 20, false, false));
 * 
 * Enable monitoring feature(isMonitoring):
 * GameContainer.instance.addChild(new Stats(150, 10, 20, false, true, true));
 * 
 * Scale it easealy(scale):
 * GameContainer.instance.addChild(new Stats(150, 10, 20, false, true, true, 2)); 
 * 
 * OR :
 * 
 * var stats:Stats = new Stats();
 * this.GameContainer.instance.addChild(stats);
 * stats.width = 150;
 * stats.x = 10;
 * stats.y = 200;
	...
	private function initMonitoring():void {
		...
		if (_isMonitoring) {
			...
			if (!this.contains(monitorView)) {
				this.stage.addChild(monitorView);
			}
